...
{
public:
  Accumulate(Callback *data, Callback *complete, Callback *error_callback,
             v8::Local<v8::Object> &options) : StreamingWorker(data, complete, error_callback)
  {
    ...
    if (options->IsObject())
    {
      v8::Local<v8::Value> filter_ = Nan::Get(options, New<v8::String>("filter").ToLocalChecked()).ToLocalChecked(); //->Get(New<v8::String>("filter").ToLocalChecked());
      if (filter_->IsString())
      {
        filter = *Nan::Utf8String(filter_);
      }
    }
  }
  ~Accumulate() {}
